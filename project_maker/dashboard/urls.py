...
from .views import UserDetails, UserTagUploadView, FearLevelUploadView, MemberShipUploadView, AchievementsUploadView, UserAchievementAPI, UserExperienceAPI, FearLevelView
urlpatterns = [
    path("users/",UserDetails.as_view(),name='user-details'),
    path('api/tags/upload/', UserTagUploadView.as_view(), name='tag-upload'),
    path('api/fear-level-upload/',FearLevelUploadView.as_view(),name="fear-level-upload"),
    path("api/fear-level/", FearLevelView.as_view(), name="fear-level"),
